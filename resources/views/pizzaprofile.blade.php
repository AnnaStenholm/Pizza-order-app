...
<head>
    ...
    <title>Pizza-king-Din beställning</title>
</head>
<body>
    <header>
      <h1>PIZZA-KING</h1>
        ...
        <div class="container pt3">
            <div class="heading"><h1 class="heading-content">I din varukorg</h1></div>
            <div class="row">
              <div class="col-sm">
               <div class="order-box">
                   <div class="order-box1">
                     <h3 class="order-eat">Du har valt: </h3>
                       <h3 class="order-eat1">{{$pizza->name}}</h3>
                    <h3 class="order-eat2">Total:  {{$pizza->price}}kr</h3>
                  </div>
                    <form action="/orderprofile" method="POST" class="form-pizza">
                      @csrf
                      <input type="hidden" name="PizzaId" value="{{$pizza->PizzaId}}" />
                    <button type="submit" class="btn btn-send">Skicka beställning</button>
                    </form>
                    <form action="/pizzas" method="POST" class="form-pizza">
                      @csrf
